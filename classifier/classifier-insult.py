...
def get_feature_matrix(tweets, perform_prints=True):
    fixed_tweets = []
...
def get_tweets_predictions(X, model, perform_prints=True):
...
if __name__ == '__main__':
    ...
    X_s = get_feature_matrix(tweets)
    y_s = get_tweets_predictions(X_s, model)

    # eval
    right_count = 0
    for i,t in enumerate(tweets):
        if tweets_class[i] == y_s[i]:
            right_count += 1
    ...
    df = pd.read_csv('../data/labeled_insult.csv')

    # ground truth
    y_t = df['Insult']

    target = df.Comment
    target = [x for x in target if type(x) == str]
    X_t = get_feature_matrix(target)

    # feature transformation

    y_pred = get_tweets_predictions(X_t, model)

    print "Printing predicted values: "
    for i,t in enumerate(target):
        print t
        print class_to_name(y_pred[i])

    is_offensive_pred = (y_pred == 1)
    accuracy = np.sum( is_offensive_pred == y_t) / float(len(df))
